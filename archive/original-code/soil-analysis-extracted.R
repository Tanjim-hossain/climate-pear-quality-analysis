...
library(scales)
library(factoextra)
pear_data <- read.csv("Data/G12.size.data.csv")
outcome_data <- read.csv("Data/G12.outcome.data.csv")
data <- read.csv("Data/G12.soil.data.csv")
cols_to_remove <- c("tree_ID", "location", "ecotr_id", "tree_id", "species", "climate")
soil_data <- select(data, -all_of(cols_to_remove))
