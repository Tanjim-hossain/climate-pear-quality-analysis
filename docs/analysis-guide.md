...
This guide explains the full workflow and identifies the relevant source for each stage. The original code, outputs and report remain available in their entirety. Values described as **reported** come from the submitted PDF; only the independent data audit and PCA/LDA checks were executed during repository preparation.

## 1. Begin with the design, not the model

Read [the assignment](project-brief.docx). It introduces future climate scenarios, Ecotron experiments, two pear varieties and the aim of selecting a favourable scenario for pear quality. The supplied brief describes experiment planning; the report and data establish the three subsequent analysis streams.
...
The submitted report, Sections 2.3 and 3.1, describes 10,000 simulations, effect sizes 10, 15 and 20, power targets around 80–90%, and an ICC of 0.20 in its selected planning example. Report Table 1 contains:
...
The [EDA notebook](../analysis/r/01-exploratory-analysis.Rmd) retains the full supplied sequence:
...
A proportion computed from a tree's records differs from a model-adjusted probability. Tree sizes vary, so averaging tree proportions and pooling all pears need not produce the same number. In the original binary bar plots, some displays retain groups that are not mapped to a visible facet; use the fully faceted location-by-species version for unambiguous subgroup interpretation. The originals are retained for traceability.

## 6. Continuous-quality mixed model

Report equation (1) models the continuous quality index using climate, species and location, plus a random tree intercept. Conference, Belgium and Scenario 1 define the reference group. The report gives an estimated tree ICC of 0.0084.
...
Report Table 3 contains the full continuous-quality coefficient comparison:
...
The report's continuous GEE QIC values are 1949.2754 for independence and 1949.3339 for exchangeable. These are nearly identical; QIC compares specified candidate models fitted to the same endpoint/data and is not a likelihood-ratio statistic.

The original workflow exports population-level and conditional fitted values, prints them, and plots predicted quality by climate. Those operations remain in [01-source-workflow.sas](../analysis/sas/01-source-workflow.sas).

## 7. Binary GLMM and GEE

Report equation (2) uses a Bernoulli response and logit link with a random tree intercept. Conditional Bernoulli variance supplies the response variability; an additional independent Gaussian residual term is not added to the logit equation.

The report's full coefficient comparison is:
...
The original notebook uses `reshape` to convert 20 weekly columns into `time` and `quality`. It retains all 3,840 scheduled observations, including missing sizes. It then calculates each pear's last observed week and the number remaining at each week, by species and climate.
...
The `na.omit` branch in the original R code operates on the wide dataset and removes an entire pear if any visit is missing. Its long export has 173 pears × 20 visits = 3,460 rows. Compare this sensitivity cohort with all available measurements, not simply with missing rows removed.
...
Report equation (3) includes fixed effects for species, climate, time, climate×time and species×time. Tree intercepts allow tree-level deviations. Pear intercepts and slopes allow correlated differences in starting size and growth rate.

Report Table 7 gives these estimates:
...
The original `.sas` uses AR(1) structure for the two random coefficients and omits species×time; the partial SAS PDF uses UN. The complete report-aligned equation is implemented separately with UN covariance. Every supplied candidate and both all-available/complete-case branches remain preserved.

The original longitudinal GEE uses normal response, identity link and AR working correlation within pear. Ordinary GEE with dropout is not automatically protected under MAR; the report's likelihood-based rationale must be understood with its assumptions.

## 11. Model comparison and diagnostics

The report compares model specifications using likelihood-based criteria and contrasts, and compares GEE working correlations with QIC. For fixed-effect comparisons using LMM likelihoods, fit comparable models with ML, not differing REML fixed-effect spaces. Variance-component boundary tests require appropriate reference distributions.

For climate-versus-reference contrasts, the report uses Bonferroni `0.05/3 ≈ 0.0167`. An omnibus equality test is distinct from an explicitly one-sided superiority hypothesis. Inspect the actual contrast signs and modelled event before interpreting a test as superiority.

The new [report-aligned SAS program](../analysis/sas/02-report-models.sas) adds explicit residual-versus-fitted and Q–Q plots for the continuous outcomes. These are additional diagnostics, not claimed completed steps of the original project. SAS logs and model fits must be reviewed when the code is executed. Similar standard errors do not prove a covariance structure is correct.

## 12. PCA on standardized soil measurements

The full code in [02-soil-analysis.R](../analysis/r/02-soil-analysis.R) was recovered from the original soil PDF, preserving every code block. Remove the six metadata variables, then use `prcomp(..., center=TRUE, scale=TRUE)` on the 17 numeric features.
...
The original analysis includes a scree plot and biplot. A dot's position is calculated from an observation's scores, never assigned randomly. Arrows describe variable directions in the displayed projection. Their interpretation depends on the biplot scaling, and a 2D plot only represents the first two components.
...
Count harvested pears per tree, join counts to the soil data, remove identifiers/group labels from predictors, and standardize the 18 predictors. Fit LDA with climate as the group label. The original workflow displays discriminant strength, LD1–LD2 scores and all LD1 coefficients.
...
The submitted analysis identifies Scenario 2 as the most consistently favourable scenario for quality and growth in its fitted models. Scenario 3 has a positive continuous-quality contrast, without comparable evidence across every endpoint. Soil variation is spread across multiple components; fitted LDA displays some group separation.

Interpretation must retain ecotron-level assignment, the lack of growth replication within climate, dropout assumptions, in-sample LDA evaluation and the source-code/report differences. The original PDF retains the full discussion, references, fixed-effect tests, contrast tables and appendices.
